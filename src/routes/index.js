...
router.get('/', function(req, res, next) {
  res.render('index', { title: 'Montoya and Jhon', state: -1, rou: 'm'});
});
...
router.post('/send', async function (req, res) {
  if(req.body.name && req.body.email && req.body.phone && req.body.comment){
    ...
    if (response === 0){
      res.render('index', { title: 'Montoya and Jhon', state: 0, rou: 's'})
    } else {
      res.render('index', { title: 'Montoya and Jhon', state: -1, rou: 's'})
    }
  }else{
    res.redirect('/')
  }
